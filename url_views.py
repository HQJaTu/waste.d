  ...
  for post in posts: # TODO pagination
    ...
    if not channel_filter or channel_filter.lower()==channel.name[1:].lower():
      if channel.private==False:
        ...
        data.append({'channelurl': channelurl, 'channel':channel,'post':post,'url':url,'extras': extras,'rates': rates})
  ...
  if url:
    ...
    for channelurl in channelurls:
      ...
      if channel.private==False:
        ...
        rating=channelurl.rating()
        #data.append({'channel':channel,'post':post,'url':url,'extras': extras})
        data.append({'channel':channel,'channelurl':channelurl,'post':post,'url':url,'extras': extras,'rates':rates,'rating':rating})
  ...
  for extra in extras:
    ...
    for post in posts:
      ...
      if channel.private==False:
        ...
        rates=Rate.query(Rate.channelurl==post.channelurl)
        d_dict={'date': post.date, 'channelurl': channelurl, 'channel':channel,'post':post,'url':url,'extras': extras,'rates': rates}
        if url not in urls:
          data.append(d_dict)
          urls.append(url)
...
def post(request,post_url,post_channel=None,post_user=None):
  ...
  try:
    taskqueue.add(name=name,queue_name='default',url='/tasks/post', params={'post_channel': post_channel,'post_user': post_user,'post_url':post_url})
...
def rate(request,post_channel,post_user,type,post_url):
  ...
  if type in types:
    ...
    try:
      taskqueue.add(name=name,queue_name='default',url='/tasks/rate', params={'channel': post_channel,'post_user': post_user,'type':type,'post_url':post_url})
    except taskqueue.TombstonedTaskError:
      logging.warning('Duplicate task name %s' % name)
...
def extra(request,post_channel,post_user,extra,post_url):
  ...
  try:
    taskqueue.add(name=name,queue_name='default',url='/tasks/extra', params={'channel': post_channel,'post_user': post_user,'extra':extra,'post_url':post_url})
  except taskqueue.TombstonedTaskError:
    logging.warning('Duplicate task name %s' % name)
  return HttpResponseRedirect('/url/')  
