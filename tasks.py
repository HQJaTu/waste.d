    ...
    if id:
      ...
      for key in url_keys:
        ...
        try:
          ...
          taskqueue.add(name=name+'_dd',queue_name='document',url='/tasks/update_document', params={'doc_id': id})
    ...
    if doc_id:
      ...
      for key in url_keys:
        ...
        try:
          prefix=self.request.get('prefix','')
          name=str(prefix)+str(doc_id)
          taskqueue.add(name=str(doc_id),queue_name='document',url='/tasks/update_document', params={'doc_id': str(doc_id)})
        except taskqueue.TombstonedTaskError:
    ...
    if doc_id:
      ...
      if urlinstance:
        ...
        if urlinstance.valid<0:
          ...
          logging.info('Delete invalid (%s) url (ID %s) from document index \'url\' (%s)' % (str( urlinstance.valid),doc_id,doc_index))
          ...
          logging.debug('doc; channel=%s, user=%s, url=%s, date=%s, title=%s, comment=%s, tag=%s, rate=%s' % (channel,user,url,date,title,comment,tag,rate))
          try:
